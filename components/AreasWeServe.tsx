...
export function AreasWeServe() {
  return (
    <section className="section-pad relative overflow-hidden"
      style={{ background:'linear-gradient(160deg,#0D2240 0%,#1A3A5C 60%,#1E4D80 100%)' }}>
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-80 h-80 rounded-full opacity-10"
          style={{ background:'radial-gradient(circle,#C84B11,transparent)', transform:'translate(25%,-25%)' }} />
        <div className="absolute bottom-0 left-0 w-64 h-64 rounded-full opacity-10"
          style={{ background:'radial-gradient(circle,#C88A10,transparent)', transform:'translate(-25%,25%)' }} />
      </div>

      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} className="text-center mb-12">
          <span className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-bold mb-5 text-white"
          ...
          <p className="text-blue-200 text-base max-w-md mx-auto">
            Covering Shankarpura and surrounding areas across Udupi district, Karnataka
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3.5">
          {areas.map((area, i) => (
            <motion.div key={area.name}
              ...
              style={area.highlight
                ? { background:'linear-gradient(135deg,#C84B11,#E05A1A)', border:'2px solid rgba(255,255,255,0.3)', boxShadow:'0 8px 32px rgba(200,75,17,0.4)' }
                : { background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.15)' }}>
              {area.highlight && (
                <span className="absolute -top-2.5 -right-2.5 px-2 py-0.5 bg-white text-[#C84B11] text-xs font-black rounded-full shadow">
                  ✦ HQ
                </span>
              )}
